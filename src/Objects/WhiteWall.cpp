...
WhiteWall::WhiteWall(float width, float height, float depth, const glm::vec3& position)
    ...
    addPrismVertices(-halfWidth, -halfHeight, halfWidth, halfHeight, 0.0f, depth, selectedColor);

    setupMesh();
    computeBoundingBox();
    loadTexture("/mnt/c/Users/jfmal/OneDrive/Documents/University/3rd Year/COS344/Homework Assignment/OpenGL-Kiosk-Rendering/src/Materials/whiteWall.jpg");
}

void WhiteWall::addPrismVertices(float x0, float y0, float x1, float y1, float z0, float z1, const glm::vec4& color) {
...
void WhiteWall::draw(const Shader& shader) const {
...
void WhiteWall::loadTexture(const char* path) {
...
void WhiteWall::computeBoundingBox() {
...
glm::vec3 WhiteWall::getBoundingBoxMin() const {
    return boundingBoxMin;
}

// Function to get maximum bounding box coordinates
glm::vec3 WhiteWall::getBoundingBoxMax() const {
